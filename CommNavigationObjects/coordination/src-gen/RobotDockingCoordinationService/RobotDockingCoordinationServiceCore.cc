...
std::string RobotDockingCoordinationServiceCore::switchCi(const std::string& ciInstanceName, const std::string& componentName, const std::string& componentInstanceName, const std::string& service, const std::string& parameter, const std::string& eventMode){
	...
	if(iter != ciInstanceMap.end()){
		...
		//std::cout<<"switchRobotDockingCoordinationService - compInstName: "<<componentInstanceName<<" parameter: "<<parameter<<" service: "<<service<<std::endl;
		...
		outString << "(error (unknown error))";
			...
			if(ACE_OS::strcasecmp(service.c_str(), "dockingEvent-activate") == 0 )
			{
				...
				param = iter->second.robotDockingCoordinationServicedockingEventEventHandlerCore->activateEventParam(parameter);
