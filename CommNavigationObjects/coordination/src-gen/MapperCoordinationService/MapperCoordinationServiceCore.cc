...
#include <smartNumericCorrelationId.h>
 ...
 int MapperCoordinationServiceCore::initCiInstance(SmartACE::SmartComponent * component, const std::string& ciInstanceName, const std::map< std::string, CiConnection, ciLessLibC > &ciConnectionsMap){
	...
	//QUERY CLIENT
	iter->second.mapperCoordinationServiceltmMapDataClient = new SmartACE::QueryClient<CommNavigationObjects::CommGridMapRequest,CommNavigationObjects::CommGridMap>(component);
	iter->second.mapperCoordinationServiceltmMapDataQueryHandler = new MapperCoordinationServiceLtmMapDataQueryHandler();
	...
	if(ci_inst_iter != ciConnectionsMap.end()){
		{
			std::map<std::string, std::string>::const_iterator service_iter = ci_inst_iter->second.serviceNameMap.find("ltmMapData");
			if(service_iter != ci_inst_iter->second.serviceNameMap.end()){
				...
				status = iter->second.mapperCoordinationServiceltmMapDataClient->connect(ci_inst_iter->second.componentInstanceName, serviceName);
 ...
 int MapperCoordinationServiceCore::finiCiInstance(const std::string& ciInstanceName){
	...
	if(iter != ciInstanceMap.end()){
			delete iter->second.mapperCoordinationServiceltmMapDataQueryHandler;
			delete iter->second.mapperCoordinationServiceltmMapDataClient;
		return 0;
	} else {
		std::cout<<"ERROR WAS NOT ABLE TO FIND CI INSTANCE IN MAP --> this should not have happend!"<<std::endl;
		return -1;
...
std::string MapperCoordinationServiceCore::switchCi(const std::string& ciInstanceName, const std::string& componentName, const std::string& componentInstanceName, const std::string& service, const std::string& parameter, const std::string& eventMode){
	...
	if(iter != ciInstanceMap.end()){
		...
		outString << "(error (unknown error))";
			...
			if(ACE_OS::strcasecmp(service.c_str(), "ltmMapData") == 0 )
			{
				CommNavigationObjects::CommGridMapRequest request;
				CommNavigationObjects::CommGridMap answer;
				
				Smart::StatusCode status;
				request = iter->second.mapperCoordinationServiceltmMapDataQueryHandler->handleRequest(parameter);
				
				status = iter->second.mapperCoordinationServiceltmMapDataClient->query(request,answer);
				outString.str("");
				switch (status)
				{
					case Smart::SMART_OK:
					{
						std::string resString = iter->second.mapperCoordinationServiceltmMapDataQueryHandler->handleAnswer(answer);
