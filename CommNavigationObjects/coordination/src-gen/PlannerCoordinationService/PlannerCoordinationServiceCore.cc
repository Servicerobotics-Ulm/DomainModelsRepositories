...
std::string PlannerCoordinationServiceCore::switchCi(const std::string& ciInstanceName, const std::string& componentName, const std::string& componentInstanceName, const std::string& service, const std::string& parameter, const std::string& eventMode){
	...
	if(iter != ciInstanceMap.end()){
		...
		//std::cout<<"switchPlannerCoordinationService - compInstName: "<<componentInstanceName<<" parameter: "<<parameter<<" service: "<<service<<std::endl;
		...
		outString << "(error (unknown error))";
			...
			if(ACE_OS::strcasecmp(service.c_str(), "plannerEvent-activate") == 0 )
			{
				Smart::StatusCode status;
				Smart::EventIdPtr id = nullptr;
				
				CommNavigationObjects::CommPlannerEventParameter param;
				param = iter->second.plannerCoordinationServiceplannerEventEventHandlerCore->activateEventParam(parameter);
			...
			if(ACE_OS::strcasecmp(service.c_str(), "plannerEvent-deactivate") == 0)
			{
				Smart::StatusCode status;
