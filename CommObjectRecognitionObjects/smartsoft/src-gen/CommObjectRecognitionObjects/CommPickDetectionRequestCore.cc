...
namespace CommObjectRecognitionObjects 
{
	...
	size_t CommPickDetectionRequestCore::generateDataHash(const DATATYPE &data)
	{
		...
		boost::hash_combine(seed, std::string(data.workspaceID.c_str()));
		boost::hash_combine(seed, std::string(data.pickmodelID.c_str()));
	...
	void CommPickDetectionRequestCore::from_xml(std::istream &is) {
		...
		if(kmp_workspaceID.search(is)) {
			std::string workspaceIDItem;
			is >> workspaceIDItem;
			setWorkspaceID(workspaceIDItem);
		}
		if(kmp_pickmodelID.search(is)) {
			std::string pickmodelIDItem;
			is >> pickmodelIDItem;
			setPickmodelID(pickmodelIDItem);
		}
	}
