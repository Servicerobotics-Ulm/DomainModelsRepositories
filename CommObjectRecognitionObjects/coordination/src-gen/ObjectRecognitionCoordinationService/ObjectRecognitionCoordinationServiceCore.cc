...
std::string ObjectRecognitionCoordinationServiceCore::switchCi(const std::string& ciInstanceName, const std::string& componentName, const std::string& componentInstanceName, const std::string& service, const std::string& parameter, const std::string& eventMode){
	...
	if(iter != ciInstanceMap.end()){
		...
		//std::cout<<"switchObjectRecognitionCoordinationService - compInstName: "<<componentInstanceName<<" parameter: "<<parameter<<" service: "<<service<<std::endl;
		...
		outString << "(error (unknown error))";
			...
			if(ACE_OS::strcasecmp(service.c_str(), "objectquery") == 0 )
			{
				...
				Smart::StatusCode status;
				request = iter->second.objectRecognitionCoordinationServiceobjectqueryQueryHandler->handleRequest(parameter);
			...
			if(ACE_OS::strcasecmp(service.c_str(), "objectrecognitionevent-activate") == 0 )
			{
				...
				param = iter->second.objectRecognitionCoordinationServiceobjectrecognitioneventEventHandlerCore->activateEventParam(parameter);
