...
std::string LocalizationCoordinationServiceCore::switchCi(const std::string& ciInstanceName, const std::string& componentName, const std::string& componentInstanceName, const std::string& service, const std::string& parameter, const std::string& eventMode){
	...
	if(iter != ciInstanceMap.end()){
		...
		//std::cout<<"switchLocalizationCoordinationService - compInstName: "<<componentInstanceName<<" parameter: "<<parameter<<" service: "<<service<<std::endl;
		...
		outString << "(error (unknown error))";
			...
			if(ACE_OS::strcasecmp(service.c_str(), "localizationEvent-activate") == 0 )
			{
				Smart::StatusCode status;
				Smart::EventIdPtr id = nullptr;
				
				CommLocalizationObjects::CommLocalizationEventParameter param;
				param = iter->second.localizationCoordinationServicelocalizationEventEventHandlerCore->activateEventParam(parameter);
			...
			if(ACE_OS::strcasecmp(service.c_str(), "localizationEvent-deactivate") == 0)
			{
				Smart::StatusCode status;
