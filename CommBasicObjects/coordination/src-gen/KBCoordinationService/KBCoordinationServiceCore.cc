...
std::string KBCoordinationServiceCore::switchCi(const std::string& ciInstanceName, const std::string& componentName, const std::string& componentInstanceName, const std::string& service, const std::string& parameter, const std::string& eventMode){
	...
	if(iter != ciInstanceMap.end()){
		...
		//std::cout<<"switchKBCoordinationService - compInstName: "<<componentInstanceName<<" parameter: "<<parameter<<" service: "<<service<<std::endl;
		...
		outString << "(error (unknown error))";
			...
			if(ACE_OS::strcasecmp(service.c_str(), "kbEvent-activate") == 0 )
			{
				Smart::StatusCode status;
				Smart::EventIdPtr id = nullptr;
				
				CommBasicObjects::CommKBEventParam param;
				param = iter->second.kBCoordinationServicekbEventEventHandlerCore->activateEventParam(parameter);
			...
			if(ACE_OS::strcasecmp(service.c_str(), "kbEvent-deactivate") == 0)
			{
				Smart::StatusCode status;
			...
			if(ACE_OS::strcasecmp(service.c_str(), "kbQuery") == 0 )
			{
				CommBasicObjects::CommKBRequest request;
				CommBasicObjects::CommKBResponse answer;
				
				Smart::StatusCode status;
				request = iter->second.kBCoordinationServicekbQueryQueryHandler->handleRequest(parameter);
