...
ManipulationCoordinationServiceIoeventEventHandlerCore::ManipulationCoordinationServiceIoeventEventHandlerCore(Smart::IEventClientPattern<CommBasicObjects::CommDigitalInputEventParameter, CommBasicObjects::CommDigitalInputEventResult> *client, std::string ciInstanceName)
: Smart::IEventHandler<CommBasicObjects::CommDigitalInputEventResult>(client)
{
	this->ciInstanceName = ciInstanceName;
}
		
void ManipulationCoordinationServiceIoeventEventHandlerCore::handleEvent(const Smart::EventIdPtr &id, const CommBasicObjects::CommDigitalInputEventResult &r) {
	...
	eventResult<< ")";
//	eventResult<<"((\"ManipulationCoordinationService\" "<<this->ciInstanceName<<" \"ioevent\" "<<id<<") "<<resultString<<")";
	int result = appedEvent(eventResult.str());
}

CommBasicObjects::CommDigitalInputEventParameter ManipulationCoordinationServiceIoeventEventHandlerCore::activateEventParam(const std::string& parameterString){
