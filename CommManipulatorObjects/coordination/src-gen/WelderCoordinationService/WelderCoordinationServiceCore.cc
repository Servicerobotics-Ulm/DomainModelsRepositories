...
#include <smartNumericCorrelationId.h>
 ...
 void WelderCoordinationServiceCore::addNewModuleInstance(const std::string& name){
 	std::cout<<"addNewModuleInstance name:"<<name<<std::endl;
	WelderCoordinationService mi;
	 ciInstanceMap.insert ( std::pair<std::string,WelderCoordinationService>(name,mi));
 }
 
 int WelderCoordinationServiceCore::initCiInstance(SmartACE::SmartComponent * component, const std::string& ciInstanceName, const std::map< std::string, CiConnection, ciLessLibC > &ciConnectionsMap){
 	std::map<std::string, WelderCoordinationService, ciLessLibC>::iterator iter = ciInstanceMap.find(ciInstanceName);
	...
	//component name: WelderCoordinationService
 ...
 int WelderCoordinationServiceCore::finiCiInstance(const std::string& ciInstanceName){
 	std::map<std::string, WelderCoordinationService>::iterator iter = ciInstanceMap.find(ciInstanceName);
	if(iter != ciInstanceMap.end()){
		return 0;
	} else {
		std::cout<<"ERROR WAS NOT ABLE TO FIND CI INSTANCE IN MAP --> this should not have happend!"<<std::endl;
...
std::string WelderCoordinationServiceCore::switchCi(const std::string& ciInstanceName, const std::string& componentName, const std::string& componentInstanceName, const std::string& service, const std::string& parameter, const std::string& eventMode){
	std::map<std::string, WelderCoordinationService>::const_iterator iter = ciInstanceMap.find(ciInstanceName);
	...
	if(iter != ciInstanceMap.end()){
		...
		//std::cout<<"switchWelderCoordinationService - compInstName: "<<componentInstanceName<<" parameter: "<<parameter<<" service: "<<service<<std::endl;
